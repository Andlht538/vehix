...
const handleCardClick = (vehicule) => {
  ...
  // Si refusé, à corriger ou doublon, rediriger vers édition
  else if (['refuse', 'a_corriger'].includes(vehicule.status)) {
    router.visit(route('vehicules.edit', vehicule.id))
  }
  // Si doublon, proposer la suppression ou voir les détails
  else if (vehicule.status === 'doublon') {
    router.visit(route('vehicules.show', vehicule.id))
  }
  // Si en attente, voir la page de statut
  else if (vehicule.status === 'en_attente') {
