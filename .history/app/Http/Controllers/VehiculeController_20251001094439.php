...
class VehiculeController extends Controller
{
    ...
    public function select(Request $request, Vehicule $vehicule)
    {
        ...
        // Only allow selection of validated vehicules
        if ($vehicule->status !== VehiculeStatus::VALIDATED) {
            return redirect()->route('vehicules.selection')
                ->with('error', 'Seuls les véhicules validécs peuvent éªtre séclectionnécs.');
        }

        // Store selected vehicule ID in session
        session(['selected_vehicule_id' => $vehicule->id]);

        return redirect()->route('dashboard')
            ->with('message', "Véhicule {$vehicule->full_name} séclectionnéc.");
    }
    ...
    public function pendingDetail(Request $request, Vehicule $vehicule)
    {
        $this->authorize('view', $vehicule);
         ...
         // Autoriser seulement les statuts non-validécs
    if ($vehicule->status === VehiculeStatus::VALIDATED) {
        return redirect()->route('vehicules.selection');
    }
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Vehicules/Index', [
            'vehicules' => $vehicules,
            'filters' => $request->only(['search', 'status']), // $vehicules est une collection paginéce
    ...
    public function store(VehiculeRequest $request)
    {
        ...
        $validationNotes = $existingVehicule
            ? "Un véhicule avec ce VIN existe décjé  dans le systé¨me (Propriéctaire: {$existingVehicule->user->name})."
            : null;
        //
        ...
        return redirect()->route('vehicules.selection')
            ->with('message', 'Véhicule crécéc avec succé¨s. ' .
                ($status === VehiculeStatus::DUPLICATE
                    ? 'Attention: Doublon déctectéc.'
    ...
    public function edit(Vehicule $vehicule)
    {
        ...
        if (!$vehicule->status->canEdit()) {
            return redirect()->route('vehicules.selection')
                ->with('error', 'Ce véhicule ne peut pas éªtre modifiéc dans son éctat actuel.');
        }
       ...
       // $vehicule->update($request->validated());
        ...
        if (in_array($vehicule->status, [VehiculeStatus::REJECTED, VehiculeStatus::TO_CORRECT, VehiculeStatus::DUPLICATE])) {
            ...
            $updateData['validation_notes'] = $existingVehicule
                ? "Un véhicule avec ce VIN existe décjé  (Propriéctaire: {$existingVehicule->user->name})."
                : null;
        ...
        return redirect()->route('vehicules.selection')
            ->with('message', 'Véhicule mis é  jour avec succé¨s. ' .
                (isset($updateData['status']) && $updateData['status'] === VehiculeStatus::DUPLICATE
                    ? 'Attention: Doublon déctectéc.'
                    : 'Soumis é  nouveau pour validation.'));
    ...
    public function destroy(Vehicule $vehicule)
    {
       ...
       return redirect()->route('vehicules.selection')
            ->with('message', 'Véhicule suppriméc avec succé¨s.');
    }
    ...
    public function validateVehicule(Request $request, Vehicule $vehicule)
    {
        ...
        // Vécrifier les permissions
        if (!$request->user()->canValidateVehicules()) {
            abort(403, 'Non autoriséc é  valider des véhicules.');
        }

        // Validation des donnéces
        $request->validate([
       ...
       $status = match($request->action) {
        ...
        $message = match($request->action) {
            'validate' => 'Véhicule validéc avec succé¨s.',
            'reject' => 'Véhicule rejetéc.',
            'correct' => 'Corrections demandéces au propriéctaire.',
        };
