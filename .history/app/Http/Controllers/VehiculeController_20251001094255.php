...
class VehiculeController extends Controller
{
    ...
    public function select(Request $request, Vehicule $vehicule)
    {
        ...
        if ($vehicule->status !== VehiculeStatus::VALIDATED) {
            return redirect()->route('vehicules.selection')
                ->with('error', 'Seuls les vÃ©hicules validÃ©s peuvent Ãªtre sÃ©lectionnÃ©s.');
        }
        ...
        return redirect()->route('dashboard')
            ->with('message', "VÃ©hicule {$vehicule->full_name} sÃ©lectionnÃ©.");
    }
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Vehicules/Index', [
            'vehicules' => $vehicules,
            'filters' => $request->only(['search', 'status']), // $vehicules est une collection paginÃ©e
    ...
    public function store(VehiculeRequest $request)
    {
        ...
        $validationNotes = $existingVehicule
            ? "Un vÃ©hicule avec ce VIN existe dÃ©jÃ  dans le systÃ¨me (PropriÃ©taire: {$existingVehicule->user->name})."
            : null;
        //
        ...
        return redirect()->route('vehicules.selection')
            ->with('message', 'VÃ©hicule crÃ©Ã© avec succÃ¨s. ' .
    ...
    public function edit(Vehicule $vehicule)
    {
        ...
        if (!$vehicule->status->canEdit()) {
            return redirect()->route('vehicules.selection')
                ->with('error', 'Ce vÃ©hicule ne peut pas Ãªtre modifiÃ© dans son Ã©tat actuel.');
        }
       ...
       // $vehicule->update($request->validated());
        ...
        if (in_array($vehicule->status, [VehiculeStatus::REJECTED, VehiculeStatus::TO_CORRECT, VehiculeStatus::DUPLICATE])) {
            ...
            $updateData['validation_notes'] = $existingVehicule
                ? "Un vÃ©hicule avec ce VIN existe dÃ©jÃ  (PropriÃ©taire: {$existingVehicule->user->name})."
                : null;
        ...
        return redirect()->route('vehicules.selection')
            ->with('message', 'VÃ©hicule mis Ã  jour avec succÃ¨s. ' .
    ...
    public function destroy(Vehicule $vehicule)
    {
       ...
       return redirect()->route('vehicules.selection')
            ->with('message', 'VÃ©hicule supprimÃ© avec succÃ¨s.');
    }
    ...
    public function validateVehicule(Request $request, Vehicule $vehicule)
    {
        ...
        if (!$request->user()->canValidateVehicules()) {
            abort(403, 'Non autorisÃ© Ã  valider des vÃ©hicules.');
        }
       ...
       $status = match($request->action) {
        ...
        $message = match($request->action) {
            'validate' => 'VÃ©hicule validÃ© avec succÃ¨s.',
            'reject' => 'VÃ©hicule rejetÃ©.',
