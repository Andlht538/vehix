  ...
  <form @submit.prevent="submitValidation" class="space-y-4">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <span class="ml-3 flex-1">
            ...
            <span class="block text-xs text-gray-600 mt-1">
              Le véhicule est conforme et peut être activé
            </span>
          </span>
        </label>
        ...
        >
          ...
          <span class="ml-3 flex-1">
            ...
            <span class="block text-xs text-gray-600 mt-1">
              Des informations doivent être corrigées par le propriétaire
            </span>
          </span>
        </label>
        ...
        >
          ...
          <span class="ml-3 flex-1">
            ...
            <span class="block text-xs text-gray-600 mt-1">
              Le véhicule ne peut pas être accepté dans le système
            </span>
          </span>
        </label>
      </div>
    ...
    <div v-if="form.action && form.action !== 'validate'">
      <label for="validation_notes" class="block text-sm font-medium text-gray-700 mb-2">
        Notes pour le propriétaire <span class="text-red-500">*</span>
      </label>
      <textarea
        id="validation_notes"
        v-model="form.notes"
      ...
      <p class="mt-2 text-xs text-gray-500">
        Ces notes seront visibles par le propriétaire. Soyez précis et constructif.
      </p>
    ...
    <div v-if="form.action === 'validate'">
      ...
      ></textarea>
      <p class="mt-2 text-xs text-gray-500">
        Vous pouvez ajouter des remarques pour le propriétaire
      </p>
    </div>
...
const getPlaceholder = () => {
  switch (form.action) {
    case 'correct':
      return 'Exemple: "Veuillez corriger le numéro VIN. Le format semble incorrect (doit contenir 17 caractères alphanumériques)."'
    case 'reject':
      return 'Exemple: "Le véhicule ne peut pas être accepté car les informations fournies ne correspondent pas aux documents standards."'
    default:
...
const submitValidation = () => {
  ...
  if (!form.action) {
    form.errors.action = 'Veuillez sélectionner une action'
    return
  }
