...
Route::middleware([
    ...
    // Routes pour les administrateurs - Gestion des demandes
Route::middleware(['auth', 'role:administrateur'])->prefix('admin')->name('admin.')->group(function () {
    // Liste des demandes de modification
    Route::get('/edit-requests', [VehiculeEditRequestController::class, 'index'])
        ->name('edit-requests.index');

    // Détails d'une demande
    Route::get('/edit-requests/{editRequest}', [VehiculeEditRequestController::class, 'show'])
        ->name('edit-requests.show');

    // Approuver une demande
    Route::post('/edit-requests/{editRequest}/approve', [VehiculeEditRequestController::class, 'approve'])
        ->name('edit-requests.approve');

    // Rejeter une demande
    Route::post('/edit-requests/{editRequest}/reject', [VehiculeEditRequestController::class, 'reject'])
