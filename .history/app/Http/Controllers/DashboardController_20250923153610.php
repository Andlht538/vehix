...
class DashboardController extends Controller
{
    ...
    private function getClientDashboardData($user)
    {
        ...
        return [
            'stats' => [
                ...
                'pendingVehicles' => $vehicules->where('status', VehiculeStatus::PENDING)->count(),
    ...
    private function getRecentActivities($user)
    {
        ...
        $repairs = $user->repairs()->with('vehicule')->orderBy('repair_date', 'desc')->take(5)->get();
        foreach ($repairs as $repair) {
            $activities->push([
                'type' => 'repair',
                'date' => $repair->repair_date,
                'description' => "Réparation sur {$repair->vehicule->full_name}",
                'amount' => $repair->cost,
            ]);
        }
        ...
        $fuelings = $user->fuelings()->with('vehicule')->orderBy('fueling_date', 'desc')->take(5)->get();
        foreach ($fuelings as $fueling) {
            $activities->push([
                'type' => 'fueling',
                'date' => $fueling->fueling_date,
                'description' => "Ravitaillement {$fueling->vehicule->full_name} ({$fueling->liters}L)",
